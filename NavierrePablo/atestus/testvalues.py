import random

class Methods:

    def randomizer(x):
        randomed = random.choice(x)
        return randomed

class Requests:

    devstage_pablo_weburl = "https://pablo-dev.vercel.app"
    prod_pablo_weburl = "https://www.navierre.com"
    dev_pablo_api = "https://api-synergyhealth-dev.osdb.io"
    stage_pablo_api = "https://api-synergyhealth-stage.osdb.io"
    headers = {"Content-Type": "application/json",
               "Accept-Encoding": "charset=utf-8",
               "Connection": "keep-alive"}
    
class Paths:

    screenpath_claimProvider1 = 'C:/Users/overk/Downloads/ClaimProvider'
    screenpath_claimPractice1 = 'C:/Users/overk/Downloads/ClaimPractice'
    screenpath_claimProvider2 = 'C:/Users/Archie-Lenovo/Downloads/ClaimProvider'
    screenpath_claimPractice2 = 'C:/Users/Archie-Lenovo/Downloads/ClaimPractice'

class General:

    npi = "1992754774"

    unique_email = ("murzilkss@mailinator.com")
    
    practice_search = ("Medical Lab", "Clinical Facility", "Rehab Center", "Great Place", "Pharmacy", "Store Goods", "Best Shelter")

    diff_values = ("123_structure_009", "-protect-55!", "way of examination", "[097513213]", "random_things_showed", "exhibition_neck",
                   "devoteToLook", "((industrials123))", "!!banish_money]}", "<<hardware_autonomy>>", "top drawer??", "fraudSSSport", "HH-8076-AA",
                   "AA-0909-BC", "77-guys-334", ">?>?>feeling", "{tedious234", "vacuous", "normal", ":::passenger:::", "license", "broad", "traumatize",
                   "sentinent", "(.<Ruler>.)", "??Trapping??", ",,Summerthing91!", "temper,,", "chieftain", "recondite", "Romantic", "Rhetorical")
    
    usPhoneNums = ("3156391271", "2067046212", "3152593482", "3152593484", "3152593485", "3152593481", "3152593478", "2035472089",
                  "2035472101", "2019058727", "4305416275", "4052955409", "6085617464", "2242864639", "4013214382", "7144856307",
                  "2148140654", "2818101867", "7632200857", "7372420880", "7146968342")
    
    firstNames = ("Allen","Baylor","Everett","Gerald","Griffin","Jamari","Kayson","Kenzo","Ronald","Thomas","Alianna","Aya","Eleanora",
                  "Elia","Halle","Jovie","Kamila","Mikayla","Rosalia","Scarlett")

    lastNames = ("Knapp","Evans","Aguirre","Frazier","Jenkins","Savage","Lucero","Atkins","Hogan","Ware","Cantu","Rogers","Fuentes",
                 "Frederick","Daniel","Pineda","Delacruz","Mullins","Reiders","Bakertin")
    
    datesOfBirth = ("09091990","11111911","12301987","09261975","11231988","01101993","07212001","03151964","08051977",
                    "01291961","10041990","02222005","04261999", "05061980", "06301945")
    
    serviceNames = ("custserv1 - reposit screening", "custserv2 > names is here", "custserv3 - development", "custserv4: acupunt nails",
                    "custserv5 > bobby doings", "custserv6 - metric accessment", "custserv7::stopping tests", "custserv8-most valuable")

    serviceDescs = ("desc Playing For Keeps", "desc ALL_SAME_TOme", "desc flea_markET_555", "desc @33In a Pickle??", "desc_Cup Of Joe_",
                    "desc A Cut Below", "desc Jaws of Death", "desc A Busy Bee", "desc Dropping Like Flies", "desc Playing For Keeps",
                    "desc Throw In the Towel", "desc A Lot on One Plate", "")

    usStates = ("Alabama","Alaska","Arizona","Arkansas","California","Colorado","Connecticut","Delaware","Florida","Georgia","Hawaii","Idaho",
                "Illinois","Indiana","Iowa","Kansas","Kentucky","Louisiana","Maine","Maryland","Massachusetts","Michigan","Minnesota","Mississippi",
                "Missouri","Montana","Nebraska","Nevada","New Hampshire","New Jersey","New Mexico","New York","North Carolina","North Dakota","Ohio",
                "Oklahoma","Oregon","Pennsylvania","Rhode Island","South Carolina","South Dakota","Tennessee","Texas","Utah","Vermont","Virginia",
                "Washington","West Virginia","Wisconsin","Wyoming")

    practiceNames = ("Autotest Practice Rehab", "Autotest Practice Estab", "Autotest Practice Crook", "Autotest Practice Medic", "Autotest Practice (Clinic)",
                     "Autotest Practice -Stater-", "Autotest Practice MedLab", "Autotest Practice WildLife", "Autotest Practice <Facilit>", "Autotest Practice",
                     "Autotest Practice .Moskito.", "Autotest Practice _Restor_")

    practiceEmails = ("petersen@maildrop.cc", "jeffcovey@maildrop.cc", "mwilson@maildrop.cc", "jdray@maildrop.cc", "shaffei@maildrop.cc",
                      "jbuchana@maildrop.cc", "nelson@maildrop.cc", "chaffar@maildrop.cc", "falcao@maildrop.cc", "csilvers@maildrop.cc",
                      "debest@maildrop.cc", "mbrown@maildrop.cc", "seanq@maildrop.cc", "garyjb@maildrop.cc", "eimear@maildrop.cc",
                      "geoffr@maildrop.cc", "milton@maildrop.cc", "bmorrow@maildrop.cc", "dofuqo@mailinator.com", "wydex@mailinator.com",
                      "morsoko1@mailinator.com", "autotesting@mailinator.com", "radomik@mailinator.com", "qwertuq@mailinator.com",
                      "ximatoz@mailinator.com", "josterko@mailinator.com", "loikolu1@mailinator.com", "macdisk@mailinator.com",
                      "poiko@mailinator.com", "dwarves1@mailinator.com", "lapadit@mailinator.com", "rolans@mailinator.com", "dryfuse@mailinator.com",
                      "iokler1@mailinator.com", "qwertus@mailinator.com", "nioxes1@mailinator.com", "zaribaydu@mailinator.com",
                      "kutsenko@mailinator.com", "mordat@mailinator.com", "glaynich@mailinator.com", "fusteri@mailinator.com")